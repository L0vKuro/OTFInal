...
import { getTwitchToken, getTwitchVideosInMonth, getYoutubeVideosInMonth, getYoutubeChannelInfo, getTwitchAvatar, monthRange } from '@/lib/sync'
...
<body style="margin:0;padding:0;background:#0D0D0D;font-family:'Arial',sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0D0D0D;padding:40px 20px;">
    ...
    // Articles — full-body content for an existing News & Updates card (matched
    // by news_id, which is the `id` field of an entry in the `news` array in
    // lib/data.ts). A news card with no row here just falls back to its excerpt
    // and the original X link on the public site.
    case 'getArticles': {
      const { data } = await supabase.from('articles').select('*').order('news_id', { ascending: false })
      return NextResponse.json({ data })
    }
    ...
    case 'upsertArticle': {
      ...
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          author: body.author || 'Overtake Staff',
    ...
    case 'deleteArticle': {
      await supabase.from('articles').delete().eq('news_id', body.news_id)
      return NextResponse.json({ success: true })
    }
